...
const SILVER = "#E8722E"; // oranye vibrant ala "Cosmic Orange" sebagai warna aksen tema terang
...
function HeroSlider({ cars, onSelectCar }) {
  ...
  return (
    <section style={{ position: "relative", width: "100%", height: "100vh", overflow: "hidden", background: car.heroStyle === "floating" ? "linear-gradient(135deg, #0d0d0d 0%, #1a1a1a 50%, #0d0d0d 100%)" : "#F5F1EA" }}>
      ...
      {car.heroStyle !== "floating" && (
        <>
          ...
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "55%", zIndex: 3, background: "linear-gradient(to top, #F5F1EA 0%, rgba(245,241,234,0.85) 18%, rgba(245,241,234,0.4) 45%, transparent 100%)" }} />
        </>
      )}
      {car.heroStyle === "floating" && (
      ...
      <button onClick={next} style={{ position: "absolute", bottom: 28, right: 48, zIndex: 10, width: 48, height: 48, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#2A2520", borderRadius: 4, cursor: "pointer", fontSize: 18, backdropFilter: "blur(4px)" }}>→</button>
...
function CarCard({ car, onView }) {
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onClick={() => onView(car)}
      style={{ background: "#FDFBF7", border: `1px solid ${hover ? GOLD + "44" : "#E5DDCC"}`, borderRadius: 10, overflow: "hidden", transition: "all 0.3s", transform: hover ? "translateY(-4px)" : "none", boxShadow: hover ? "0 20px 60px rgba(0,0,0,0.5)" : "none", cursor: "pointer" }}>
      <div style={{ position: "relative", paddingTop: "62%", overflow: "hidden", background: "#EFE9DC" }}>
        ...
        {sold && (
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ border: "3px solid #fff", color: "#2A2520", fontWeight: 900, fontSize: 22, padding: "6px 24px", transform: "rotate(-8deg)", letterSpacing: "0.1em" }}>SOLD</div>
          </div>
        )}
      </div>
      <div style={{ padding: "18px 20px 20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid #2a2a2a", paddingTop: 14 }}>
          <div>
            <div style={{ color: "#4A3F2E", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>Harga</div>
          ...
          <button onClick={(e) => { e.stopPropagation(); onView(car); }} disabled={sold}
            style={{ padding: "9px 18px", background: sold ? "#D5CDB8" : GOLD, color: sold ? "#6A5F4E" : "#F5F1EA", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 12, cursor: sold ? "default" : "pointer", letterSpacing: "0.04em" }}>
...
function CatalogSection({ cars, onView }) {
  ...
  const pillStyle = (active) => ({ padding: "7px 18px", borderRadius: 99, border: `1.5px solid ${active ? GOLD : "#D5CDB8"}`, background: active ? GOLD : "transparent", color: active ? "#F5F1EA" : "#7A6F5E", cursor: "pointer", fontSize: 13, fontWeight: active ? 700 : 500, transition: "all 0.2s" });

  return (
    <section id="katalog" style={{ background: "#EFE9DC", padding: "60px clamp(16px, 4vw, 48px)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h2 style={{ color: "#2A2520", fontSize: "clamp(28px, 4vw, 46px)", fontWeight: 800, margin: 0, letterSpacing: "-0.02em" }}>Katalog Kendaraan</h2>
        </div>

        <div style={{ background: "#FDFBF7", border: "1px solid #2a2a2a", borderRadius: 10, padding: "20px 24px", marginBottom: 40, display: "flex", flexWrap: "wrap", gap: 20, alignItems: "flex-end" }}>
          <div style={{ flex: "1 1 200px" }}>
            <div style={{ color: "#4A3F2E", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Cari</div>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Merek atau model..." style={{ width: "100%", background: "#F0EAD9", border: "1px solid #333", borderRadius: 6, padding: "9px 14px", color: "#2A2520", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
          </div>
          <div>
            <div style={{ color: "#4A3F2E", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Merek</div>
          ...
          <div>
            <div style={{ color: "#4A3F2E", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Tipe</div>
          ...
          <div style={{ flex: "0 0 200px" }}>
            <div style={{ color: "#4A3F2E", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Maks. Harga: <span style={{ color: GOLD }}>{fmtShort(maxPrice * 1e6)}</span></div>
        ...
        <div className="zm-grid-catalog" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 24 }}>
          ...
          {filtered.length === 0 && (
            <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "80px 0", color: "#9A8F7E" }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>🚗</div>
              <div style={{ fontSize: 18, color: "#4A3F2E" }}>Tidak ada kendaraan yang cocok</div>
            </div>
          )}
        </div>
      </div>
...
function InspectionReport({ inspection }) {
  ...
  return (
    <div>
      <div style={{ background: "#FDFBF7", border: `1px solid #2a2a2a`, borderRadius: 12, padding: 28, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            <div style={{ color: "#7A6F5E", fontSize: 12, marginBottom: 4 }}>Total Titik Inspeksi</div>
            <div style={{ color: "#2A2520", fontSize: 28, fontWeight: 800 }}>{totalPoints}+ <span style={{ fontSize: 14, color: "#5A4F3E", fontWeight: 500 }}>titik diperiksa</span></div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ color: "#7A6F5E", fontSize: 12, marginBottom: 4 }}>Kondisi Baik</div>
        ...
        <div className="zm-grid-inspect" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
          {summary.map(cat => (
            <button key={cat.key} onClick={() => setOpenCat(cat.key)}
              style={{ background: openCat === cat.key ? `${statusColor[cat.worst]}15` : "#F0EAD9", border: `1.5px solid ${openCat === cat.key ? statusColor[cat.worst] : "#E5DDCC"}`, borderRadius: 10, padding: "16px 14px", cursor: "pointer", textAlign: "center", transition: "all 0.2s" }}>
      ...
      <div style={{ background: "#FDFBF7", border: "1px solid #2a2a2a", borderRadius: 12, padding: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
          <span style={{ fontSize: 22 }}>{INSPECTION_CATEGORIES.find(c => c.key === openCat).icon}</span>
          <h3 style={{ color: "#2A2520", margin: 0, fontSize: 18, fontWeight: 700 }}>{INSPECTION_CATEGORIES.find(c => c.key === openCat).label}</h3>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {inspection[openCat].map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 16px", background: "#F0EAD9", borderRadius: 8, border: `1px solid ${statusColor[item.status]}22` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: "#6A5F4E", fontSize: 12.5 }}>{item.note}</div>
              </div>
            </div>
          ))}
        </div>
      </div>
      <p style={{ color: "#9A8F7E", fontSize: 11.5, marginTop: 16, lineHeight: 1.6 }}>
...
function Gallery({ images }) {
  ...
  return (
    <div>
      <div style={{ position: "relative", paddingTop: "62%", borderRadius: 12, overflow: "hidden", marginBottom: 12, background: "#EFE9DC" }}>
      ...
      <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingRight: 16, WebkitOverflowScrolling: "touch" }}>
        {images.map((img, i) => (
          <button key={i} onClick={() => setActive(i)} style={{ flexShrink: 0, width: 88, height: 60, borderRadius: 6, overflow: "hidden", border: `2px solid ${active === i ? GOLD : "transparent"}`, padding: 0, cursor: "pointer", background: "#EFE9DC" }}>
...
function InstantCheckout({ car, onSubmit }) {
  ...
  const inp = { background: "#F0EAD9", border: "1px solid #333", borderRadius: 6, padding: "11px 14px", color: "#2A2520", fontSize: 14, outline: "none", width: "100%", boxSizing: "border-box" };
  ...
  if (sent) return (
    <div style={{ background: "#FDFBF7", border: `1px solid ${GOLD}44`, borderRadius: 12, padding: 36, textAlign: "center" }}>
      <div style={{ fontSize: 48, marginBottom: 16 }}>✅</div>
      <h3 style={{ color: "#2A2520", margin: "0 0 10px", fontSize: 20, fontWeight: 700 }}>Pesanan Diterima!</h3>
      <p style={{ color: "#7A6F5E", fontSize: 14, lineHeight: 1.6 }}>Tim Sales kami akan menghubungi Anda dalam 1 jam kerja untuk verifikasi data dan langkah selanjutnya.</p>
    </div>
  );
  ...
  return (
    <div style={{ background: "#FDFBF7", border: "1px solid #2a2a2a", borderRadius: 12, padding: 28 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid #2a2a2a" }}>
        ...
        <div>
          <h3 style={{ color: "#2A2520", margin: 0, fontSize: 18, fontWeight: 700 }}>Beli & Kirim ke Rumah</h3>
          <div style={{ color: "#5A4F3E", fontSize: 12, marginTop: 2 }}>Tanpa survei tatap muka — proses online penuh</div>
        </div>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div><label style={{ color: "#5A4F3E", fontSize: 12, display: "block", marginBottom: 6 }}>Nama Lengkap (sesuai KTP) *</label>
          <input style={inp} value={form.nama} onChange={e => setForm(f => ({ ...f, nama: e.target.value }))} placeholder="John Doe" /></div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div><label style={{ color: "#5A4F3E", fontSize: 12, display: "block", marginBottom: 6 }}>No. HP / WhatsApp *</label>
            <input style={inp} value={form.hp} onChange={e => setForm(f => ({ ...f, hp: e.target.value }))} placeholder="08xxxxxxxxxx" /></div>
          <div><label style={{ color: "#5A4F3E", fontSize: 12, display: "block", marginBottom: 6 }}>Email</label>
            <input style={inp} value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="email@contoh.com" /></div>
        </div>

        <div><label style={{ color: "#5A4F3E", fontSize: 12, display: "block", marginBottom: 6 }}>Alamat Pengiriman (lengkap) *</label>
        ...
        <div>
          <label style={{ color: "#5A4F3E", fontSize: 12, display: "block", marginBottom: 6 }}>Unggah Foto KTP</label>
          <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => handleKtpUpload(e.target.files[0])} />
          <button onClick={() => fileRef.current.click()} disabled={uploadingKtp} style={{ width: "100%", padding: "12px", background: "#F0EAD9", border: `1.5px dashed ${form.ktp ? GOLD : "#D5CDB8"}`, borderRadius: 6, color: form.ktp ? GOLD : "#5A4F3E", fontSize: 13, cursor: uploadingKtp ? "default" : "pointer" }}>
            {uploadingKtp ? "⏳ Mengunggah..." : form.ktp ? `✓ ${form.ktpFileName}` : "📷 Klik untuk unggah foto KTP"}
          </button>
        </div>

        <div>
          <label style={{ color: "#5A4F3E", fontSize: 12, display: "block", marginBottom: 8 }}>Metode Pembayaran</label>
          <div style={{ display: "flex", gap: 10 }}>
            {["Cash", "Ajukan Kredit"].map(m => (
              <button key={m} onClick={() => setForm(f => ({ ...f, metode: m }))} style={{ flex: 1, padding: "12px", border: `1.5px solid ${form.metode === m ? GOLD : "#D5CDB8"}`, borderRadius: 6, background: form.metode === m ? `${GOLD}1a` : "transparent", color: form.metode === m ? GOLD : "#7A6F5E", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
        ...
        {form.metode === "Ajukan Kredit" && (
          <div style={{ background: "#EFE9DC", border: "1px solid #2a2a2a", borderRadius: 8, padding: 14, color: "#6A5F4E", fontSize: 12.5, lineHeight: 1.6 }}>
        ...
        <div style={{ background: "#EFE9DC", border: "1px solid #2a2a2a", borderRadius: 8, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ color: "#7A6F5E", fontSize: 13 }}>Unit dipesan</span>
          <span style={{ color: GOLD, fontWeight: 700, fontSize: 14 }}>{car.brand} {car.model}</span>
        </div>

        <button onClick={handleSubmit} disabled={submitting || uploadingKtp} style={{ width: "100%", padding: "15px", background: (submitting || uploadingKtp) ? "#4A3F2E" : GOLD, color: "#F5F1EA", border: "none", borderRadius: 6, fontWeight: 800, fontSize: 15, cursor: (submitting || uploadingKtp) ? "default" : "pointer", letterSpacing: "0.04em", textTransform: "uppercase", marginTop: 6 }}>
          {submitting ? "Mengirim..." : uploadingKtp ? "Tunggu unggah KTP..." : "Pesan Sekarang →"}
        </button>
        <p style={{ color: "#9A8F7E", fontSize: 11, textAlign: "center", margin: 0 }}>Dengan memesan, Anda menyetujui untuk dihubungi tim Sales kami via WhatsApp/telepon.</p>
      </div>
    </div>
  );
}
...
function MiniKredit({ price, carName }) {
  ...
  return (
    <div style={{ background: "#FDFBF7", border: "1px solid #2a2a2a", borderRadius: 12, padding: 24 }}>
      <div style={{ color: "#7A6F5E", fontSize: 12, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>Tanya Harga & Cicilan</div>
      <p style={{ color: "#8A7F6E", fontSize: 13, lineHeight: 1.6, margin: "0 0 18px" }}>Tim sales kami siap membantu hitungkan simulasi cicilan sesuai DP dan tenor yang Anda inginkan.</p>
      <a href={waLink} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", padding: "13px 8px", background: "transparent", border: `1px solid ${SILVER}`, borderRadius: 4, color: "#2A2520", fontSize: 11, fontWeight: 400, letterSpacing: "0.06em", textTransform: "uppercase", textDecoration: "none", boxSizing: "border-box", whiteSpace: "nowrap" }}>
...
function VerifyPage({ txId }) {
  ...
  const row = (label, value) => (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #E5DDCC" }}>
      <span style={{ color: "#7A6F5E", fontSize: 13 }}>{label}</span>
      <span style={{ color: "#2A2520", fontSize: 13, fontWeight: 600, textAlign: "right", maxWidth: "60%" }}>{value || "-"}</span>
    </div>
  );

  return (
    <div style={{ minHeight: "100vh", background: "#F5F1EA", padding: "100px clamp(16px, 4vw, 48px) 60px", boxSizing: "border-box" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <img src="/zahramobil/logo.png" alt="Zahra Mobil" style={{ height: 56, marginBottom: 12 }} />
          <h1 style={{ color: "#2A2520", fontSize: 22, fontWeight: 700, margin: 0 }}>Verifikasi Kwitansi</h1>
          <p style={{ color: "#7A6F5E", fontSize: 13, marginTop: 6 }}>Hasil scan QR Code dari kwitansi pembayaran Zahra Mobil</p>
        </div>

        {status === "loading" && (
          <div style={{ textAlign: "center", color: "#7A6F5E", padding: "60px 0" }}>Memverifikasi data...</div>
        )}

        {status === "notfound" && (
          <div style={{ background: "#FDFBF7", border: "1px solid #d97a7a", borderRadius: 12, padding: 28, textAlign: "center" }}>
            <div style={{ fontSize: 36, marginBottom: 10 }}>⚠️</div>
            <div style={{ color: "#c0392b", fontWeight: 700, fontSize: 15, marginBottom: 6 }}>Kwitansi Tidak Ditemukan</div>
            <div style={{ color: "#7A6F5E", fontSize: 13 }}>Data transaksi ini tidak ada dalam sistem kami. Kwitansi mungkin tidak sah atau telah dihapus.</div>
          </div>
        )}

        {status === "error" && (
          <div style={{ background: "#FDFBF7", border: "1px solid #d97a7a", borderRadius: 12, padding: 28, textAlign: "center" }}>
            <div style={{ color: "#c0392b", fontSize: 13 }}>Terjadi kesalahan saat memverifikasi. Coba scan ulang atau hubungi showroom.</div>
          </div>
        )}

        {status === "found" && tx && (
          <>
            <div style={{ background: "#FDFBF7", border: "1px solid #b8e0b8", borderRadius: 12, padding: "14px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 20 }}>✅</span>
              <span style={{ color: "#2e8b3a", fontWeight: 700, fontSize: 14 }}>Kwitansi Ini Valid & Tercatat di Sistem Zahra Mobil</span>
            </div>

            <div style={{ background: "#FDFBF7", border: "1px solid #E5DDCC", borderRadius: 12, padding: 24, marginBottom: 16 }}>
            ...
            {car && (
              <div style={{ background: "#FDFBF7", border: "1px solid #E5DDCC", borderRadius: 12, padding: 24 }}>
            ...
            <p style={{ color: "#8a7f6e", fontSize: 11, textAlign: "center", marginTop: 24 }}>
  ...
  return (
    <div style={{ background: "#F5F1EA", minHeight: "100vh" }}>
      <div style={{ padding: "100px clamp(16px, 4vw, 48px) 0", maxWidth: 1200, margin: "0 auto", boxSizing: "border-box" }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#7A6F5E", fontSize: 13, cursor: "pointer", marginBottom: 24, display: "flex", alignItems: "center", gap: 6 }}>← Kembali ke Katalog</button>
        ...
        <div className="zm-grid-detail" style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 32, marginBottom: 40 }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 20, borderBottom: "1px solid #2a2a2a" }}>
              {[["galeri", "📷 Galeri Foto"], ["inspeksi", "🔍 Laporan Inspeksi"], ["spesifikasi", "📋 Spesifikasi"]].map(([key, label]) => (
                <button key={key} onClick={() => setTab(key)} style={{ padding: "12px 18px", background: "none", border: "none", borderBottom: `2.5px solid ${tab === key ? GOLD : "transparent"}`, color: tab === key ? GOLD : "#6A5F4E", fontSize: 13.5, fontWeight: 600, cursor: "pointer" }}>
                  {label}
                </button>
              ))}
            </div>
              ...
              <div style={{ background: "#FDFBF7", border: "1px solid #2a2a2a", borderRadius: 12, padding: 28 }}>
                {[["Merek", car.brand], ["Model", car.model], ["Tipe Bodi", car.type], ["Tahun", car.year], ["Kilometer", `${car.km.toLocaleString("id-ID")} km`], ["Warna", car.color], ["Transmisi", car.transmission], ["Bahan Bakar", car.fuel], ["No. Rangka", car.noRangka], ["No. Mesin", car.noMesin]].map(([label, val], i) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "13px 0", borderBottom: i < 9 ? "1px solid #2a2a2a" : "none" }}>
                    <span style={{ color: "#6A5F4E", fontSize: 13.5 }}>{label}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20, minWidth: 0 }}>
            ...
            {car.status !== "Terjual" ? (
              ...
              <div style={{ background: "#FDFBF7", border: "1px solid #2a2a2a", borderRadius: 12, padding: 28, textAlign: "center" }}>
                <div style={{ fontSize: 36, marginBottom: 12 }}>🔒</div>
                <div style={{ color: "#8A7F6E", fontSize: 14 }}>Unit ini sudah terjual</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 32, marginBottom: 40 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              ...
              <span style={{ color: "#2A2520", fontWeight: 700, fontSize: 18 }}>ZAHRA <span style={{ color: GOLD }}>MOBIL</span></span>
            </div>
            <p style={{ color: "#4A3F2E", fontSize: 13, lineHeight: 1.7, maxWidth: 280 }}>Dealer mobil bekas terpercaya dengan inspeksi 150+ titik dan jaminan transparansi penuh.</p>
          </div>
          {[["Layanan", ["Beli & Kirim ke Rumah", "Laporan Inspeksi Digital", "Konsultasi via WhatsApp", "Ajukan Leasing"]], ["Kontak", ["📍 Jl. Dr. Mr. Mohd Hasan, Batoh, Kec. Lueng Bata, Kota Banda Aceh", "💬 0811-6707-099"]]].map(([title, items]) => (
            <div key={title}>
              ...
              {items.map(item => <div key={item} style={{ color: "#5A4F3E", fontSize: 13, marginBottom: 8, lineHeight: 1.5 }}>{item}</div>)}
            </div>
          ))}
        </div>
        <div style={{ borderTop: "1px solid #1a1a1a", paddingTop: 24, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <span style={{ color: "#D5CDB8", fontSize: 12 }}>©2026 SRISP. All rights reserved.</span>
          <span style={{ color: "#D5CDB8", fontSize: 12 }}>Powered by Firebase × Next.js</span>
        </div>
      </div>
    </footer>
  );
...
// ─── APP ─────────────────────────────────────────────────────────────────────
export default function ZahraMobilGuestLight({ onToggleTheme }) {
  ...
  return (
    <div style={{ background: "#F5F1EA", minHeight: "100vh", fontFamily: "'Poppins', 'Inter', system-ui, sans-serif", color: "#2A2520", overflowX: "hidden", width: "100%", maxWidth: "100vw", boxSizing: "border-box" }}>
      <style>{`
      ...
      <a href="https://wa.me/628116707099" target="_blank" rel="noreferrer" aria-label="Hubungi via WhatsApp" style={{ position: "fixed", bottom: 28, right: 28, zIndex: 999, width: 54, height: 54, background: "#FDFBF7", border: `1px solid ${SILVER}`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 8px 24px rgba(42,37,32,0.25)", textDecoration: "none", transition: "border-color 0.2s" }}>
